...
## 롬복과 최신 트렌드 (@RequiredArgsConstructor)
1. 롬복 라이브러리 추가 및 설정
    1. 설정
    - build.fradle
        - configuration {
...
1. DiscountService는 Map으로 모든 DiscountPolicy 를 주입 
    - 빈(@Component)으로 등록된 fixDiscountPolicy , rateDiscountPolicy 가 주입
...
​
    ...
    // DiscountService는 Map으로 모든 DiscountPolicy 를 주입 (fixDiscountPolicy , rateDiscountPolicy 가 주입)
