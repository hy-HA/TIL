...
식별자가 필요하고, 지속해서 값을 추적, 변경해야 한다면 그것 은 값타입이 아닌 엔티티
```
# 엔티티 타입
- @Entity로 정의하는 객체
...
# 값 타입 - 임베디드 타입(embedded type, 복합 값 타입)
## 임베디드 타입
...
# 값 타입 - 값 타입 컬렉션 (collection value type)

## 1. 값 타입의 비교
    - 인스턴스가 달라도 그 안에 값이 같으면 같은 것으로 봐야함
    1. 동일성(identity) 비교 : 인스턴스 참조값을 비교. == 사용
    2. 동등성(equicalence) 비교 : 인스턴스의 값을 비교, equals()
...
member.setHomeAdress(new Address("newCity","street","10000"));
...
em.persist(member);
//member만 persist해도 컬랙션은 다른 테이블이지만 같은 라이프사이클로 돌아감. 값 타입은 본인 스스로 라이프사이클을 갖지 못함. 생명주기가 member에 소속됨
```
2. 값 타입 조회 예제
...
Set<String> favoriteFoods = findMember.getgetFavoriteFoods();
for (String favoriteFoods : favoriteFoods) {
    sout("favoriteFoods=" + favoriteFoods)
}
```
3. 값 타입 수정 예제
...
```
//homeCity > newCity
Address a = findMemebr.getHomeAddress();
